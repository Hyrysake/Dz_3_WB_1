...
def sort_file(item_path):
    file_name, file_ext = os.path.splitext(os.path.basename(item_path))
    new_file_name = normalize(file_name)
    new_file_ext = file_ext.lower()
    new_item_name = new_file_name + new_file_ext
    new_item_path = os.path.join(os.path.dirname(item_path), new_item_name)
    ...
    category_path = os.path.join(os.path.dirname(item_path), category)
    ...
    with ThreadPoolExecutor() as executor:
        ...
        for item in os.listdir(folder_path):
            ...
            if os.path.isfile(item_path):
                futures.append(executor.submit(sort_file, item_path))
            elif os.path.isdir(item_path) and item not in EXTENSIONS_DICT:
                futures.append(executor.submit(sort_folder, item_path))
